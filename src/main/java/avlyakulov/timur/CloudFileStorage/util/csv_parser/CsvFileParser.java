...
public class CsvFileParser {
    ...
    private static void fillFileFormats()  {
        ...
        InputStream is = classloader.getResourceAsStream("file_icon.csv");
        try {
            assert is != null;
            try (CSVReader reader = new CSVReader(new BufferedReader(new InputStreamReader(is)))) {
                String[] nextRecord;
                reader.readNext();
            ...
            throw new RuntimeException("something went wrong with icon file " + e.getMessage());
        }
    }
